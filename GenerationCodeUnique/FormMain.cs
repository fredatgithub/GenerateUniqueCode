...
namespace GenerationCodeUnique
{
    public partial class FormMain : Form
    {
        ...
        {
            textBoxCodeTiers.Text = GenerateCode();
        }
        ...
        private string GenerateRandomCharacters(int numberOfCharacter, List<string> authorizedCharacters)
        {
            string result = string.Empty;
        ...
        private IEnumerable<string> AddNumbers()
        {
        ...
        private IEnumerable<string> AddCharacters()
        {
        ...
        private bool IsAcceptable(string mot, int min, int max, List<string> forbiddenCharacters)
        {
        ...
        {
            string premierElement = GenerateCode();
            listBoxSequenceSuivie.Items.Add(premierElement);
            string deuxiemeElement = GenerateNextItem(premierElement);
            listBoxSequenceSuivie.Items.Add(deuxiemeElement);

        }

        private string GenerateNextItem(string premierElement)
        {
            string result = string.Empty;
            char firstChar = premierElement[premierElement.Length - 1];
            // test if GetNextSymbol(firstChar) == '0' or 'a' or 'A' then GetNextSymbol(secondChar)
            result = premierElement.Substring(0, premierElement.Length - 1) + GetNextSymbol(firstChar);
            return result;
        }

        private char GetNextSymbol(char firstChar)
        {
        ...
        private char GetNextNumber(char number)
        {
            char nextNumber;
            if (number == '9')
            {
